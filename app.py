...
logger = logging.getLogger('evelyn-api')
    ...
    try:
        ...
        async with AsyncSession(impersonate=bp['impersonate'], verify=False, timeout=90) as session:
            ...
            try:
                ...
                logger.error(f"[JSON PARSE FAIL] Status={response.status_code if response else 'N/A'} | Length={len(resp_text) if resp_text else 0} | First500={repr(resp_text[:500]) if resp_text else 'EMPTY'}")
            ...
            if 'actionreq' in final_lower or 'action_required' in final_lower:
                return True, f"3DS_REQUIRED", gateway, total_price, currency
...
async def _throttled_process(cc, mes, ano, cvv, site_url, variant_id, proxy_list, start_proxy_idx=0):
    ...
    async with sem:
        ...
        try:
            ...
            # Safety fallback — should never reach here, but prevents NoneType crash
            return False, "All retries exhausted", "UNKNOWN", "0.00", "USD"
        finally:
            async with get_stats_lock():
                _active_cards -= 1
                _total_processed += 1
